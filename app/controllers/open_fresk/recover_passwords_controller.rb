class RecoverPasswordsController < ApplicationController
  ...
  def user_id_from_reset_token
    $redis.get("fresqueclimat:users:reset_password_token:#{@recovery_token}")
  end

  def verify_token
    redirect_to new_forgot_password_path, alert: t("password_recovery_invalid") unless @current_user
    nil
  end
